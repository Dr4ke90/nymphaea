  ...
  async () => {
    try {
      const response = await axios.get(
        "http://localhost:3000/api/nymphaea/sales"
      );
    ...
    if (Object.keys(response.data.response).length !== 0) {
      ...
      if (sale.produse && sale.produse.length > 0) {
          ...
          if (productInSale.reteta) {
            for (const itemReteta of productInSale.reteta) {
              ...
              if (inventoryItem) {
                ...
                promises.push(
                  axios.put(
                    `http://localhost:3000/api/nymphaea/inventory/${itemReteta.cod}`,
                    updatedInventory
                  )
                );
              }
            ...
            if (inventoryItem) {
              ...
              promises.push(
                axios.put(
                  `http://localhost:3000/api/nymphaea/inventory/${productInSale.cod}`,
                  updatedInventory
                )
              );
            }
       ...
       if (responsePromise.request.status === 200) {
          await axios.delete(`http://localhost:3000/api/nymphaea/casa/${sale.nrBon}`)
       }
      }
    }
